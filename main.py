...
def main():
    ...
    # # The fast_tokenizer is implemented with RUST
    ...
    def print_trainable_named_parameters(model):
        ...
        print_rank_0(f"Number of trainable parameters: {len(trainable_params)}")
        for name, param in trainable_params:
            print_rank_0(f"Parameter name: {name}, Shape: {param.shape}")

    print_trainable_named_parameters(rm_model)
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if args.l1_lambda:
                # Compute the L1-norm of rm_model
                l1_norm = sum(torch.abs(param).sum() for name, param in l1regularized_params)
    ...
    if args.output_dir is not None:
        ...
        for name, param in l1regularized_params:
            ...
            large_params += torch.sum(param.abs() > 1e-7).item()

        sparsity = (nonzero_params / total_params) * 100
        sparsity_large = (large_params / total_params) * 100

        print_rank_0(f"self sparsity:{sparsity}", args.global_rank)
        print_rank_0(f"self sparsity rough:{sparsity_large}", args.global_rank)
        ...
        for name, param in l1regularized_params:
            ...
            for threshold in thresholds:
                significant_count = (param.abs() > threshold).sum().item()
                sparsity_ratios[threshold] = (significant_count / total_count) * 100
                print_rank_0(
                    f"self sparsity rough (1e-{int(-1 * torch.log10(torch.tensor(threshold)).item())}): {sparsity_ratios[threshold]:.6f}",
                    args.global_rank,
                )

        max_abs_value = max(param.abs().max().item() for param in rm_model.parameters())
        print_rank_0(f"Maximum absolute value of model parameters:{max_abs_value}", args.global_rank)
